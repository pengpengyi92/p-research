...
- **与我们匹配**：**B+（强）**——RL 经验命中 RLHF 要求 + harness 工程命中 Agent 安全 + 研究驱动文化契合
...
- **扁平化**、结果导向、无层级内耗/无效加班
- 创新源于自主与无后顾之忧
- 使用 Claude Code/Cursor（AI-native 开发文化——与我们契合）

## 6. 与我们匹配度（B+ 强）

| 维度 | 方寸需要 | 我们 | 匹配 |
|---|---|---|---|
| **RL 经验** | RLHF/DPO | **RL 四轮迭代实证** | ⭐⭐⭐ 直接命中 |
| **Agent 工程** | Agent 安全/架构 | **dsh-quant harness + 自动化闭环** | ⭐⭐⭐ |
| **AI-native 文化** | Claude Code 日常 | 我们就是 AI-native | ⭐⭐⭐ |
| Benchmark | 安全评测基准 | 归因/benchmark 方法论 | ⭐⭐ |
| 安全专精 | 攻防/红队 | 无专精（可学）| ⭐ |
| 顶会论文 | 加分项 | 无 | ⭐（非必需）|

**结论：B+（强）——比方寸智能/方寸无忧匹配度明显更高**：
1. RL 经验直接命中 RLHF 要求（少见的强匹配）
2. harness 工程 = Agent 安全的基础技能
3. AI-native 文化天然契合（扁平 + Claude Code）
4. 研究驱动（清华系）= 赤诚原则契合
